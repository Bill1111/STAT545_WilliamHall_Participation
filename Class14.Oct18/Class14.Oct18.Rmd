...
ggplot(mtcars, aes(wt, mpg)) +
  geom_point() +
  labs(x = "Weight (1000's of lbs)") #so we can go up to the imaginary line, then go across to the y axis
```
...
```{r}
head(gapminder)
(my_lm <- lm(lifeExp ~ log(gdpPercap), data=gapminder))#this is just the linear regression
unclass(my_lm) #this gives you all the residuals of each data point compared to the predicted line
names(my_lm) #this is all the names/stuff in the regression
my_lm$coefficients #this just gives you the coefficients
summary(my_lm) #this gives you everything :)
```
...
```{r}
predict(my_lm) %>% 
  head() #these are the y values that we are getting form the log of the first 6 values in gdpPercap

head(gapminder$gdpPercap) #these are the first 6 x values that it is using to plug into the formula

(log(779.4453)*8.4 + -9.1) #this is the math

#Could also choose your own data you wanted to

a <- tibble(gdpPercap = c(500,600,700))

predict(my_lm, newdata = a) 


#So if you wanted to do that prediction thing, what you would do is create a dataset that is just the averages of the data that you had. Then you would run that through the algorythm, and it would forecast the average sales or whatever#
...
ggplot(gapminder, aes(gdpPercap, lifeExp)) +
    geom_point() + #this will give you the points
    geom_smooth(method="lm") + #this wil give you the line automatically
    scale_x_log10() #this will put the x values on a log scale

#This will only give you the picture but it won't give you the numbers
```
...
tidy(my_lm) #statistic is the t stat, pval
augment(my_lm, newdata = a) #fitted gives ou predicted values, lifeExp tells you what they actually were, residial are in the .resid column, standard error of fitted values in .se.fit. Can also do this with new data fucntion to do it with new data that you want to specify.
glance(my_lm)
