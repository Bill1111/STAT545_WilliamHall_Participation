...
a <- c(1,2,3)
b <- c(4,5,6)
map2(a, b, function(x, y) x*y) #this multiplies each of the first numbers in the column together. Rather than writing out the function before hand and calling it, we are writing it directly into the map2() function

```
```{r}
map2(a, b, ~ .x * .y) #can do this too

```
```{r}
map2(a, b, `*`) #So we can find the multiply function by putting `` around the multiple sign.
```
```{r}
map2_dbl(a, b, `*`) #This will give us a numeric vector output
```
...
b <- c(4,5,6)
c <- c(7,8,9)
pmap(list(a, b, c), function(x, y, z) x*y*z) #this multiplies all the first ones together.
```
```{r}
...
2. By defining a function "on the fly" (in the map2 function) within the `map2()` function.

```{r}
map2_chr(commute, name, function(t, s) str_c(s, " takes ", t, " minutes to get to work."))
```
...
```{r}
got_chars[[1]] #this is all the content of the first car in this list -> it is theon greyjoy

got_alias <- got_chars %>% {
  tibble(name = map_chr(., "name"), #the map_chr() output has to be a character of length 1
         aliases = map(., "aliases")) #map is more flexible
}

got_alias #this is the full tibble
...
```{r}
(alias_list <- got_alias$aliases) #so you make a list of all the aliases first
names(alias_list) <- got_alias$name #then you name each set of characters with the names from the got_alias
alias_list
...
got_alias %>%
  ...
  `[[`("Melisandre") #this is the name as x[["Melisandre"]]
```
...
```{r}
icecream %>% 
    unnest(flav) #this makes each of those different ice cream flavours a different row with the same person's name
```
