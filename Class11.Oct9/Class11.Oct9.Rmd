...
```{r}
a <- 3
print(typeof(a)) #how man decimals can be stored?
print(mode(a)) 
print(class(a))
```
...
is.numeric(a) #this will tell you if it's numeric or not.
is.data.frame(a)
```
...
```{r}
as.character(100) #this will make a string with the digits 100
as.numeric("100") #this will change the digits 100 to the number 100
...
course[c(3,1)] #this will output the 3rd and 1st entry
...
String = one obects with multiple character in it
Vector = group of multiple objecters
...
a ^ 2 #this will square each component of the vector
...
a * 1:11 #because these don't match, it will go 1:10 and give you the error message.
...
a * 1:3 #10 not divisble by 3 so it goes through, but then gives you an error message
...
a > 0 # this is a logical vector because it contain's logicals = true, false
...
a[1:n] <- 1:2 #this gives us 1,2 repeated for 10 times
a
```
...
setNames(course, c("subject", "code", "section")) #this gives you column headers for the vector
...
(a <- 7:-2)
(n <- length(a))
a > 0 #this will give you trues and falses for objects greater than 0
a[a > 0] #this will give you the objects in your vector that are true
```

## Lists

Unlike vectors, which are atomic/homogeneous, a list in R is heterogeneous. They can only types of objects. 

Try storing the course code (`"STAT"` and `545`) again, but this time in a list. Use the `list()` function.

```{r}

course <- list("STAT", 545) #makes two different vectors, and can hold anything.
course
```
...
```{r}
(my_list <- list(year=2018, instructor=c("Vincenzo", "Coia"), fav_fun=typeof)) #you can store numbers, character vectors, and functions in a list. It's like a train with different cars.
```
...
my_list[1] #this gives you the first car ie. the whole car
my_list[[1]] #this gives you the contents of the the first car
my_list[2] # this gives you the second vector/train care
my_list[[2]][2] #this gives you the second car and the second piece of content
...
my_list["year"]
my_list[["year"]]
my_list$year #gives you what is in that vector from that 
