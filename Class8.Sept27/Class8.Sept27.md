...
`mutate()` - this will make a new column with what is after the '=' sign, so you can see that we made a new column named 'gdp' and another column named 'gdpBill'
-----------------------------------------------------------------------------------------------------------------------------------------------------------------
...
Try the same thing, but with `transmute` (drops all other variables). This will drop all other varibles except for the ones that you created
...
Example: Suppose Canada's 1952 life expectancy was mistakenly entered as 68.8 in the data frame, but is actually 70. Fix it using `if_else` and `mutate`. If something in letters is equal to "h", then we want to replace it with "H". Otherwise just leave it as what it was.

The other one is: we want to change the Life Expectancy Column only for those rows that are Canada and year is 1952 to the value "70". If we don't have that then Life Expectancy is just what it was before.
...
For each continent: What is the smallest country-wide median GDP per capita? Need to calculate the median GDP per Country -&gt; then find the min
...
![](Class8.Sept27_files/figure-markdown_github/unnamed-chunk-9-1.png)
...
``` r
gapminder %>% 
  mutate(senior = if_else(lifeExp > 65, "yes", "no")) %>% 
  group_by(continent, senior) %>% 
  summarize(md = median(gdpPercap)) %>% 
  arrange(md)
```
...
gapminder %>% 
  group_by(country) %>% 
  mutate(growthPeryear = pop -lag(pop))
```

    ## # A tibble: 1,704 x 7
    ## # Groups:   country [142]
    ##    country     continent  year lifeExp      pop gdpPercap growthPeryear
    ##    <fct>       <fct>     <int>   <dbl>    <int>     <dbl>         <int>
...
Notice the NA's because there is nothing before that number.
