...
``` r
#This will only colour points where the LifeExp is greater than 60#
...
Facetting - make a plot for each category then put them together
----------------------------------------------------------------
...
``` r
gvsl + geom_point(aes(size = pop, fill=continent), shape = 21, alpha = 0.5) +
...
Path plots - connects dots in order that they appear by year
------------------------------------------------------------
...
``` r
gvsl + geom_point(alpha=.2) + geom_smooth(method="lm")
```

![](cm007-exercise_files/figure-markdown_github/unnamed-chunk-16-1.png)

``` r
gvsl + geom_point(alpha=.2) + coord_polar()
```
...
ggplot(gapminder, aes(x=year, y=pop)) +
  ...
  geom_smooth(method='lm', aes(colour=country), se=FALSE)
```
