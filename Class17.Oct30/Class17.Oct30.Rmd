...
- hold multiple entries of the same type - characters, numerics
...
- hold multiple entries of anything - they don't all have to be the same
- no entries are coerced (as a result of being able to hold anything)
- subset with `[` - first car, `[[` - content of first car in train, or `$`. 
...
```{r}
#This will square everything between 1 and 10
(1:10) ^ 2
```

```{r}
#This will multiply 1 by 1, 2 by 2, 3 by 1, 4 by 2, etc.
(1:10) * (1:2)
```
...
So instead of having a nice dataframe now you have nested data. Lists within lists.

1. `str()`: embrace `list.len` - how many elements you want to see, and `max.level`
...
```{r}
x <- 1:10
sqrt(x) #this is vectorization
map(x, sqrt) #so this will run the sqrt function on all elements of x
```
...
Want a vector to be returned (instead of a list which is the default? Must specify the `typeof()` of the vector. Use `map_dbl()` to specify an output vector of type "double" for the above (check out the documentation `?map` for the acceptable vector types):

```{r}
map_dbl(x, sqrt) #This will make a numeric vector of the output

map_chr(x, sqrt) #This will make a character vector
```

Does your function have other arguments? You can specify them afterwards in the ellipses (`...`).

```{r}
map_chr(x, str_c, "potato.", sep="-") #this will make a character vector of what you want. Notice the str_c() embedded here.
```
...
1. Let's retry the `wesanderson` example: use `purrr` to write code that extracts a vector of the first elements of each vector contained in `wesanderson`. Play around with the following, too:
    ...
    - Use the ` ``[`` ` function if you're feeling daring - the `[`(x,1) is the same as x[1]
...
```{r}
map_chr(wesanderson, 1) # %>% unname() -> this will get rid of the names
```
...
map_dfr(got_chars, `[`, desired_info) #making a dataframe out of this stuff UUGGGEE
```
