...
lotr2 <- "https://raw.githubusercontent.com/jennybc/lotr-tidy/master/data/The_Return_Of_The_King.csv" %>% 
  read.csv()
str(lotr1) # First 2 variables are Factors with Levels. Levels are unique entries.
str(lotr2)
```
...
```{r}
iris %>% 
  mutate(Species = ifelse(Species == "versicolor", "vers", Species)) #so this is if we wnat to change versicolour into vers. If we do this it will change the other levels in this variable to numbers rather than leave them as the word. 
```

```{r}
c(iris$Species, "setosa") #gives you integers then gives you setosa as word
```
...
```{r}
set.seed(10) #the set.seed sets the random draw from the sample
(draw <- sample(letters[1:3], size = 10, replace = TRUE)) #this is a character vector (not a factor)
```
...
draw <- as.factor(draw)
draw #notice that this now has the levels whereas the character one just had the characters

levels(draw) #what are the levels
nlevels(draw) #what is the number of levels
fct_count(draw) #gives us a cute tibble that breaks everything down
...
```{r}
c(lotr1$Film, lotr2$Film) #if we try to bind the variables in these two datasets we just get some ones which doesn't make sense.

fct_c(lotr1$Film, lotr2$Film) #this will add both of these levels together.
...
nlevels(gap_gs$country) #there should on be two, but there is still 142. All the levels are still being stored.
levels(gap_gs$country) #this is even though the only levels that appear are Gernamny and Sweden
(gap_gs$country)
```
...
gap_small %>% 
  ...
  geom_bar() +
  scale_x_discrete(drop=FALSE) + #this keeps the levels that are zero
...
```{r}
#So let's drop the continents we don't want

gap_small %>% 
  droplevels() %>% #if there is no observation in the column, then it will drop that level
  str()

gap_small %>% 
  mutate(continent = fct_drop(continent)) %>% #if there is no observation in the column, then it will drop that level. Note that country still has all the levels.
  str()
```
...
Reorder by frequency instead of by alphabetical order:

- Rearrange by frequency: `fct_infreq()` will automatically reorder based on frequency
- Reverse: `fct_rev()` 

```{r, fig.width=2, fig.height=1}
cont %>% 
  fct_infreq() %>% #instead of alphabetical order we now have descending order
  qplot()
```
...
```{r}
#put them in the order that they appear in the levels
draw %>%
  fct_inorder()
```
...
#this sort of gives you a hodge podge of dots
```
...
```{r, fig.width=2, fig.height=1}
gapminder %>% 
  mutate(continent = fct_reorder(continent, lifeExp)) %>% #order continent by median life expectancy
...
#This would be better if we re-ordered the lines.
```
...
```{r, fig.width=2, fig.height=1}
gap_select %>% 
  mutate(country = fct_reorder2(country, year, lifeExp)) %>% #organize country by year and life expectancy
...
gap_asia_2007$country %>% 
  fct_relevel("Syria","Sweden", after=2) %>% #this puts syria and sweden in the second position
  levels() %>% 
  head()
```
...
gap_big_north$country %>% 
  fct_recode("USA" = "United States") %>% #could also use mutate here mutate( "United States" = "USA")
  levels()
```
...
```{r, fig.width=2, fig.height=1}
cont %>% 
  fct_collapse("combo" = c("Europe", "Asia")) %>% #coool making a combo of Europe and Asia
  qplot()
```
...
```{r, fig.width=2, fig.height=1}
cont %>% 
  fct_lump() %>% #Combined all the that weren't Africa and Asia and Europe into other
  qplot()

cont %>% 
  fct_lump(n=2) %>% #only keep the top two, then lump everything else
  qplot()
